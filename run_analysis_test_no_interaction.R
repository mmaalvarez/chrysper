...
grna_library = vroom(paste0(rawPath, "4_resources/crispr_libraries/brunello/original/broadgpp-brunello-library-contents_gRNAs.tsv")) %>%
  rename(Target_Gene_Symbol = gene) %>%
  # update Non-Targeting sgRNAs name (remove trailing " Control" part)
  mutate(Target_Gene_Symbol = gsub(" Control$","", Target_Gene_Symbol)) %>%
  # combine sgRNA and gene into an id
  unite(col = "sgRNA_id", sgRNA, Target_Gene_Symbol, sep = "__", remove = F) %>%
  # keep interesting columns
  select(sgRNA_id, sgRNA, Target_Gene_Symbol, "sgRNA Target Sequence", "Target Context Sequence", "PAM Sequence", "Strand", "Genomic Sequence", "Position of Base After Cut (1-based)") %>%
...
raw_counts_offset = raw_counts_t %>%
  # long format
  gather(key = "sample",
         value = "counts",
         -c(sgRNA, gene)) %>%
...
raw_counts_for_eset = raw_counts_t %>%
  # combine sgRNA and gene into an id
  unite(col = "sgRNA_id", sgRNA, gene, sep = "__") %>%
  # sgRNA_id to row names
  column_to_rownames("sgRNA_id") %>%
...
# #assigning treatment contrast matrix to eset data (contrasts is inside pData(eset))
...
## save processed data
save(eset,
     file = paste0(dataDir, "eset.RData_t"))
...
gene_names = fData(eset) %>%
  ...
  distinct() %>%
  pull(gene) #pull similar to $

treatment_names = pData(eset) %>%
  select(treatment_recat) %>%
  distinct() %>%
  pull(treatment_recat)


NBres = mclapply(treatment_names, function(t){
  mclapply(gene_names, function(g){

  df = buildDF(g, eset, conditions, raw_counts_offset)
  
  # it only works if tretament levels are 'treated' or 'control' (try to generalize)
  if(t == 'treated'){
    # from general dataframe, subset two dataframes: control and treatment
    df_treated =
      # choose only rows with treated or time_0 (to add time_0 to treated samples)
      df[df$treatment_recat == 'treated' | df$time_cat == 'time_0',] %>%
      # change time_0 treatment_recat to 'treated'
      mutate(treatment_recat = replace(treatment_recat, treatment_recat == 'control','treated')) %>%
      arrange(time_cat)
    y_treated = fitModel(g, eset, conditions, df_treated)}
  else{
    df_control = df[df$treatment_recat == 'control', ]
    # run NB regression on that gene
    y_control = fitModel(g, eset, conditions, df_control)}

}, mc.cores = 1)}, mc.cores = 1)


# NBres is now a list of lists. Convert it into a single (each list element is a treatment-gene pair)
NBres = do.call(c,NBres)

# name elements by their analyzed gene's name
# duplicate gene_names because for each gene we have a pair control-treated
# the order is: first all controls and then all treated
names(NBres) = c(paste0(gene_names,"_control"),
                 paste0(gene_names,"_treated"))

## save processed data
save(NBres, file = paste0(dataDir, "NBres_t.RData"))
...
# apply broom::tidy() to each gene from NBres (shows only estimate, std. error, statistic and p.value)
NBres_tidy = NBres %>%
  map(., ~tidy(.x$model)) %>%
  # remove sgRNA rows
...
allres = eat(NBres_tidy[[1]],
  ...
  gather(key = "estimate.SE.stat.pval",
         value = "value",
         -term) %>%
  # split 'estimate.SE.stat.pval' into 'estimate.SE.stat.pval', 'gene' and 'treatment' columns
  ...
  # THE ONLY TESTED CONDITION HERE IS TIME SO WE DON'T NEED THIS COLUMN
  ...
  mutate(fdr = p.adjust(p.value,
                        method = "BH")) %>%
  # order and sort
  select(gene, treatment, term, estimate, std.error, statistic, p.value, fdr) %>%
  arrange(gene,treatment)
  #arrange(fdr, p.value)

## write table to file
write_tsv(allres,
          paste0(tabDir, "results_t.tsv"))
...
# define hits:
  # control: time_catmiddle-vs-zero ~ time_catlate-vs-middle ~ 0 (slope ~ 0)
  # treatment: time_catmiddle-vs-zero ~ time_catlate-vs-middle < 0 (slope neq 0)
# tolerance for the slope interval
tol = 0.1
hits = allres %>%
  filter(ifelse(treatment == 'control',
           yes = abs(estimate) < 0.1,
           no = fdr < 0.05 &
             estimate > -1-tol &
             estimate < -1+tol))
for(g in gene_names){
  ifelse(nrow(hits[hits$gene == g, ]) == length(unique(allres$term))*length(treatment_names),
  yes = filter(hits, hits$gene == g),
  no = hits[hits$gene != g,])
}
...
for(gene in unique(hits$gene)){
  data_glm = NBres[[gene]]$df
  # #plot counts against time_cat prediction
  # plot_model(NBres[[gene]]$model, type = "pred", terms = c("time_cat", "treatment_recat")) +
  #   ggtitle(gene)
  
  # plot number of counts vs time including the regression
  p_counts_vs_time = ggplot(data_glm,aes(x = time_cat,
                                     y = `real counts`,
                                     color = treatment_recat)) + 
    geom_point() +
    # summarise y values at unique x value
    # add points for the means
    stat_summary(aes(group = treatment_recat),
                     geom = 'point',
                     fun = 'mean',
    ...
    ggtitle(gene) +
    theme_minimal() 
  show(p_counts_vs_time)
  ggsave(path = figDir, filename = paste0(gene, "_counts_vs_time_test.jpg"),
         plot = p_counts_vs_time, device = "jpg", width = 10, height = 5.6, dpi = 300)
  
    
  
  # # plot sgRNA real (raw) counts per gene×treatment×time
  # x_data_1 = data_glm$'treatment_recat'
  # y_data_1 = data_glm$'real counts'
  # p_real_counts = ggplot(data_glm,
  #                        aes(x_data_1,y_data_1,
  #                            color = "sgRNA")) +
  #   geom_boxplot() +
  #   facet_grid(facets = ~time_cat) +
  #   theme_bw() +
  #   ggtitle(gene)
  # ggsave(path = figDir, filename=paste0("gene", "_raw_grna_counts_t.jpg"),
  ...
  # x_data_2 = data_glm$'real counts'
  # y_data_2 = data_glm$'fitted counts'
  # p_real_vs_fitted_counts = ggplot(data_glm,aes(x_data_2,y_data_2,group = "id")) +
  #   geom_point() +                                     #to scatter plot (points appear)
  #   geom_smooth(method = "lm",formula = y~x) +                      #to add a line with specified slope and intercept to the plot
  ...
  #   theme_minimal() +
  #   ggtitle(gene)
  # ggsave(path = figDir, filename = paste0("gene", "_raw_vs_fitted_grna_counts_t.jpg"),
  #        plot = p_real_vs_fitted_counts, device = "jpg", width = 10, height = 5.6, dpi = 300)
}

# # heatmap sample×sgRNAs
# pdf(paste0(figDir, "top_hits_heatmap_t.pdf"), width=10, height=5.6)
# heatmap.2(exprs(eset[fData(eset)$gene %in% hits$gene, ]),
...
# ## miguel attempt to plot
# # which gene
# gene = "A1BG"
# # extract df for that gene in NBres
# NBres[[gene]]$df %>%
...
#              # better check fitted counts
#              y = `fitted counts`,
#              color = treatment_2_levels)) +
#   # violin plot
#   geom_violin(width = 0.2,
#               position = position_dodge(width = 0.3)) +
#   # boxplot within the violin plot
